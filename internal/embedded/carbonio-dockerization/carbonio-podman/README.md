...
To support both k3s and podman, through kubernetes syntax using `play 
kube`, avoiding duplicating yaml definitions, the new `start.sh` is the 
standard way of running the apps. 
It requires installing helm in order to generate the yaml definitions. 
The benefit is you can customize the docker images and values you want to 
use, without changing the source code, a feature that is present on docker 
compose through .env files. 

## Startup

Start everything: `./start.sh up`

Customize startup with specific groups: `VALUES_FILE=custom_values.yaml ./start.sh up`
...
## Teardown
Stop: `./start.sh down`
Stop and remove volumes: `./start.sh down --force`

Run `./start.sh help` to see the usage helper.
