...
# Important: zimbraSmtpHostname must match the hostname of postfix container
docker exec "$MAILBOX_CONTAINER" sh -c "> /tmp/prov.ls && cat > /tmp/prov.ls <<EOF
...
mc default carbonioWscRecordingEnabled TRUE
