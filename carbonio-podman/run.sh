...
collect_volume_claims() {
  local dir="$1" f claim
  for f in "$dir"/*.yaml; do
    [ -f "$f" ] || continue
    while IFS= read -r claim; do
      [ -n "$claim" ] || continue
      contains "$claim" "${VOLUME_CLAIMS[@]}" || VOLUME_CLAIMS+=("$claim")
    done < <(sed -n 's/^[[:space:]]*claimName:[[:space:]]*//p' "$f")
  done
}

remove_volume_claims() {
  local claim
  [ "$#" -eq 0 ] && return 0
  echo "==> Removing volumes: $*"
  for claim in "$@"; do
    podman volume rm -f "$claim" >/dev/null 2>&1 || true
  done
}
...
case "${1:-help}" in
  ...
  down)
    ...
    for arg in "${@:2}"; do
      case "$arg" in
        ...
        *) echo "Unknown arg: $arg (expected: all|mails|files|wsc|--force|--deep)" >&2; exit 1 ;;
      esac
    done
    if [ $DEEP -eq 1 ] && [ -z "$FORCE" ]; then
      echo "--deep requires --force" >&2
      exit 1
    fi
    ...
    if [ $DEEP -eq 1 ]; then
      echo "==> Tearing down ${SELECTED_GROUPS[*]} (removing volumes, deep cleanup)..."
      VOLUME_CLAIMS=()
      for group in "${SELECTED_GROUPS[@]}"; do
        collect_volume_claims "$group"
      done
    elif [ -n "$FORCE" ]; then
    ...
    if [ $DEEP -eq 1 ]; then
      remove_volume_claims "${VOLUME_CLAIMS[@]}"
    fi
    if [ $FULL_TEARDOWN -eq 1 ]; then
  ...
  *)
    echo "Usage: $0 {up [groups...] [--multi] [--missing] | down [groups...] [--force] [--deep] | status | stats | logs <pod> [container] | restart <yaml>}"
