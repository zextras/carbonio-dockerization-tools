...
func (e *Extractor) extractAll() error {
	...
	// Walk through embedded filesystem (skip the "cli" directory itself)
	err := fs.WalkDir(EmbeddedFiles, ".", func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return err
		}

		// Skip the cli directory and its contents
		if path == "cli" || strings.HasPrefix(path, "cli/") || strings.HasPrefix(path, "cli\\") {
			if d.IsDir() {
				return fs.SkipDir
			}
			return nil
